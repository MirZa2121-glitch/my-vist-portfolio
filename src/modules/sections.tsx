                ...
                <div className='py-10'>
                    <div className='flex flex-col lg:flex-row items-center justify-between gap-10'>
                        <div className='w-full lg:max-w-[720px] pt-10'>
                            <p className='w-45 h-8 my-5 flex items-center justify-center text-[14px] font-semibold rounded-4xl bg-[hsl(0,0%,91%)] text-[#6D5EF9]'>{t('intro.role')}</p>
                            <h1 className='intro__title text-[44px] sm:text-[52px] lg:text-[60px] font-semibold leading-[1.05]'>{t('intro.titleStart')} <abbr className='text-[#6D5EF9]'>{t('intro.titleAccent')}</abbr> {t('intro.titleEnd')}</h1>
                            <p className='intro__desc py-8 sm:py-10 text-[16px] sm:text-[18px] leading-8 text-[#667085]'>{t('intro.desc')}</p>
                            <div className='flex flex-col sm:flex-row items-start sm:items-center gap-4'>
                                <a href='#portfolio' className='w-full sm:w-60 h-15 flex items-center justify-center rounded-[10px] bg-[#6D5EF9] text-[#F8FAFC]'>{t('intro.viewProjects')}</a>
                                <a href='#about' className='w-full sm:w-40 h-15 flex items-center justify-center rounded-[10px] border-2 border-[#6D5EF9] text-[#6D5EF9]'>{t('intro.about')}</a>
                            </div>
                            <div className='mt-10'>
                                <p className='pb-5 text-[#667085] font-semibold'>{t('intro.technology')}</p>
...
export const About = function() {
    ...
    const blocks = [
        {id: 1, number: '1', name: t('about.blocks.clear'), desc: t('about.blocks.clearDesc')},
        {id: 2, number: '2', name: t('about.blocks.adaptive'), desc: t('about.blocks.adaptiveDesc')},
        {id: 3, number: '3', name: t('about.blocks.clear'), desc: t('about.blocks.clearDesc')},
        {id: 4, number: '4', name: t('about.blocks.clear'), desc: t('about.blocks.clearDesc')},
    ]
                ...
                <div className='flex lg:items-center flex-col lg:flex-row items-start gap-10'>
                    <div className='w-full lg:w-185'>
                        <div className='w-full max-w-full lg:max-w-[560px]'>
                            <p className='w-full sm:w-30 h-7 my-5 rounded-[100px] font-semibold flex items-center justify-center text-[14px] text-[#6D5EF9] bg-[#6e6e6e40]'>{t('about.label')}</p>
                            <h3 className='about__title text-[32px] sm:text-[36px] lg:text-[40px] font-semibold text-[#F8FAFC]'>{t('about.title')}</h3>
                            <p className='about__desc text-[15px] sm:text-[16px] py-5 leading-7 text-[#667085]'>{t('about.desc')}</p>
                        </div>
                    </div>
        ...
        <section id="portfolio" className="portfolio__block">
            <div className="w-full lg:max-w-7xl mx-auto px-4 py-16 md:py-24">
                <div className="max-w-2xl mb-12">
                    <p className='w-30 h-7 my-5 rounded-[100px] font-semibold flex items-center justify-center text-[14px] text-[#6D5EF9] bg-[#6e6e6e40]'>{t('portfolio.label')}</p>
                    <h2 className="portfolio__title text-3xl md:text-4xl font-bold mb-4 text-[#1a1a1a]">{t('portfolio.title')}</h2>
                </div>
                        ...
                        {$port.map($card => (
                            <SwiperSlide key={$card.id}>
                                <a href={$card.href} className="portfolio__card block rounded-2xl border border-[#d1d1d1] bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                                    ...
                                    <p className="text-sm text-[#4a4a4a] leading-5">{t(`projects.${$card.id}`, { defaultValue: $card.desc || t('portfolio.empty') })}</p>
                                    <p className='my-5 font-semibold text-[#6D5EF9] hover:underline'>{$card.underText || t('portfolio.readMore')}</p>
                                </a>
                            </SwiperSlide>
                        ))}
                    </Swiper>
            ...
            <div className='w-full lg:max-w-7xl mx-auto px-4 py-16 md:py-24 bg-[#0D111B] rounded-[10px]'>
                <div className='flex flex-col lg:flex-row items-center justify-center gap-10 px-6 lg:px-10'>
                    <div className='w-full lg:w-115'>
                        <p className='w-full sm:w-40 h-8 my-5 rounded-[100px] font-semibold flex items-center justify-center text-[14px] text-[#6D5EF9] bg-[#6e6e6e40]'>{t('ares.label')}</p>
                        <h3 className='about__title text-[32px] sm:text-[36px] lg:text-[40px] font-semibold text-[#fd0707]'>Ares</h3>
                        <p className='about__desc text-[15px] sm:text-[16px] py-5 leading-7 text-[#667085]'>{t('ares.desc')}</p>
                        <button className='w-full sm:w-45 h-13 border-2 font-semibold border-[#fd0707] rounded-[10px] text-[#d1d1d1]'>{t('ares.soon')} 🔒</button>
                    </div>
                ...
                <div>
                    <div>
                        <div className='max-w-2xl mb-10 px-2 sm:px-0'>
                            <h2 className='works__title text-2xl sm:text-3xl md:text-[35px] font-bold text-[#1a1a1a]'>{t('works.title')}</h2>
                                ...
                                {$allPort.map($cardPort => (
                                    <SwiperSlide key={$cardPort.id} className='works__slide'>
                                        <a href={$cardPort.href} className="block rounded-2xl border border-[#d1d1d1] bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md max-w-sm mx-auto">
                                            ...
                                            <h4 className="text-base sm:text-lg font-semibold text-[#1a1a1a] mb-2">{$cardPort.name || t('works.project')}</h4>
                                            <p className="text-sm text-[#4a4a4a] leading-5">{t(`allProjects.${$cardPort.id}`, { defaultValue: $cardPort.desc || t('works.empty') })}</p>
                                        </a>
                                    </SwiperSlide>
                                ))}
                            </Swiper>
            ...
            <div className='w-full lg:max-w-7xl mx-auto px-4 py-16 md:py-14'>
                <div className='my-30'>
                    <div className='w-full lg:w-130'>
                        <p className='w-full sm:w-40 h-8 my-5 rounded-[100px] font-semibold flex items-center justify-center text-[14px] text-[#6D5EF9] bg-[#6e6e6e40]'>{t('stack.label')}</p>
                        <h4 className='text-[32px] sm:text-[36px] lg:text-[40px] font-semibold text-[#0D111B]'>{t('stack.title')}</h4>
                        <p className='text-[15px] sm:text-[16px] py-5 leading-7 text-[#667085]'>{t('stack.desc')}</p>
                    </div>
                ...
                <div className='rounded-[32px] border border-[#E8EAF8] bg-[#F8FAFF] p-6 shadow-sm'>
                    <div className='flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between'>
                        <div className='max-w-2xl'>
                            <p className='inline-flex rounded-full bg-[#F0F4FF] px-4 py-2 text-[14px] font-semibold text-[#5B63F6]'>{t('another.label')}</p>
                            <h4 className='mt-6 text-[30px] font-semibold text-[#0D111B]'>{t('another.title')}</h4>
                        </div>
